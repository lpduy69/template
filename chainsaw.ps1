...
$repo_path = "C:\chainsaw_x86_64-pc-windows-msvc\chainsaw\sigma"
...
# Windows Sigma Path
$windows_path = "C:\chainsaw_x86_64-pc-windows-msvc\chainsaw\sigma\rules\windows"

# Run Chainsaw and store JSONs in TMP folder
& 'C:\chainsaw_x86_64-pc-windows-msvc\chainsaw\chainsaw.exe' hunt C:\Windows\System32\winevt -s $windows_path --mapping 'C:\chainsaw_x86_64-pc-windows-msvc\chainsaw\mappings\sigma-event-logs-all.yml' --from $from --output $env:TMP\chainsaw_output\results.json --json --level high --level critical
